...
Phase 2a done (2026-09-05): 12 MCP tools (`council_models/ask/probe/plan/dispatch/status/answer/cancel/review/verdict/merge/handoff`), `council init/doctor/events` CLI, agents `council-planner/reviewer/integrator`, `UserPromptSubmit` hook, gates (`before_review`, `after_merge`), routing by task type incl. new `assets` role (DESIGN §19.12). Verified live (Phase 1): Codex + local Ollama in parallel → `review`, blocked → answer → resume. Unit-tested only: merge, conflict re-dispatch, reject → attempt+1 → failed at 3. Not yet: repo publication (user decision), live review→merge run, Gemini login (CLI installed), Phase 2b (`trust`, `defects_after_merge`, playbooks, `/council:compare`, `/council:why`, `LESSONS.md`). Environment: `docs/probe-2026-09.md`; lessons: DESIGN §19.11.
