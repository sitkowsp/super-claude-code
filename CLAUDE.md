...
- **Raster assets (§19.14)**: only Codex generates PNG/JPG (built-in image tool on ChatGPT login); `prompt_cli.j2` tells `assets` tasks to copy the image into the workdir and verify it. Gemini needs an API key for images; Copilot cannot.
