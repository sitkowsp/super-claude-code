...
Phase 1 done (2026-09-05): 8 MCP tools (`council_models/ask/probe/plan/dispatch/status/answer/cancel`), `council init/doctor` CLI, plugin commands in `commands/council/`, CI/LICENSE/CONTRIBUTING/SECURITY written. Verified live: Codex + local Ollama in parallel → `review`, blocked → answer → resume. Not yet: publishing the repo (user decision), Gemini `run` (CLI not installed), `HANDOFF.md`, Phase 2 (reviewer/integrator subagents, `review`/`merge`, gates, hook). Environment facts: `docs/probe-2026-09.md`; live-test lessons: DESIGN.md §19.11.
