...
tools: Read, Grep, Glob, Bash, mcp__council__council_review, mcp__council__council_verdict, mcp__council__council_ask
---
...
5. For `implement` tasks with more than ~200 changed lines, ask a second model via `council_ask`
   ...
   answer; you decide.
6. Call `council_verdict(task, ok, reason)`. A rejection reason is written for the executor: what is
   wrong, what to change, what to leave alone. It becomes their ANSWER.md.
