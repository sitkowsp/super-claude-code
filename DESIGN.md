...
2026-09-08 (rev. 3.25: adapter `cursor` — Cursor CLI (`cursor-agent`) jako opcjonalny wykonawca, domyślnie wyłączony, wykrywany przez doctor/sondę; headless `-p --output-format text --force`, czyta AGENTS.md/CLAUDE.md, `--model` zamiast pokrętła effortu, `cursor-agent models` zasila dzienną sondę; zweryfikowane tylko wykrywanie — brak konta Cursor na maszynie dev (§29, rc26); rev. 3.24: dzienna sonda dostępności — `probe_ttl_hours` (domyślnie 24) i `probe.ensure_fresh` przed każdym dispatch: przestarzałe `capabilities.json` wymusza ponowną sondę wszystkich dostawców (z przywróceniem `enabled` z council.json, więc dostawca może wrócić); odkrywanie listy modeli per CLI (`agy models` działa; reszta cicho odmawia — config autorytatywny) i rejestr effortów per adapter w `capabilities.json`; `council_models` z wiekiem sondy i ostrzeżeniem o modelu spoza odkrytej listy (§28, rc25); rev. 3.23: automat oceny złożoności karty (`complexity.assess`, deterministyczny) → tier modelu (`ModelConfig.tier`, prosty→low/cheap, złożony→high/codex+fable) i effort per dispatch (codex `model_reasoning_effort`, claude-sub `--effort`; `Task.complexity/effort`, powody w evencie `dispatched`, podgląd w `council_plan`, wyłącznik `delegation.auto_effort`) (§27, rc24); rev. 3.22: `docs/manual.md` (pełny podręcznik komend: gdzie działa która komenda i co robić per stan), `write_kit` dopisuje do README vaulta blok `<!-- council:readme -->` z tabelą Claudian-vs-sesja (rc22); rev. 3.21: reconcile liczy merged+oszczędności z diffu brancha (wykonawca zrobił pracę, choć chair scalił ręcznie); backfill rozumie eventy `reconciled`; CLI `council savings [--backfill]` (rc21); rev. 3.20: `reconcile` — recenzowane zadania scalone ręcznie poza radą (treść bajt w bajt na base) zamykane jako merged bez bramek/commitów/oszczędności: `council_merge(reconcile=true)`, `/council:merge --reconcile`, CLI `council reconcile` (one-shot obok żywej sesji); recenzent nie odrzuca już `done_without_changes` w tym przypadku (rc20, przypadek merit-db); rev. 3.19: izolacja symulacji od vaulta — `COUNCIL_OBSIDIAN_VAULT=off` wyłącza warstwę Obsidian, a repo w katalogu tymczasowym OS nigdy nie mirroruje do auto-wykrytego vaulta (§20, rc19; wycieki chair-repo/acct-repo); rev. 3.18: blok `Council status (auto)` w Dashboard.md vaulta — `obsidian.update_dashboard` przepisuje sekcję między markerami `<!-- council:auto -->` przy każdym mirrorze (stany per projekt, oszczędności, czas ostatniej notatki; treść użytkownika poza markerami nietknięta, rc18); rev. 3.17: mirror Obsidiana przy każdej zmianie stanu zadania (`TaskStore.on_transition` → `_mirror`, rc17) — dashboard nie pokazuje już nieaktualnego stanu między plan/merge/handoff; rev. 3.16: profile kont Claude dla wykonawców, łańcuchy fallbacku per model, czas resetu limitu (§26, rc16); rev. 3.15: szacunek zaoszczędzonych tokenów (§25, rc15); rev. 3.14: opcje nadzorcy — `chair.plan_assist/review_assist/coder`, model `fable` (Claude `--effort medium`), `COUNCIL_EXECUTOR`, `/council:chair` (§24, rc13); rev. 3.13: wykrywanie Unreal na dowolnym dysku stałym (rc12, 19.17); rev. 3.12: Codex domyślnie GPT-6 Astra (`reasoning`, `context_window`), rola `3d` + wykrywanie Blender/Unreal + playbook `game-assets` (19.17, rc11); rev. 3.11: T-003 — render headless Edge z workdira Codexa jest zawodny (T-001 zadziałał, T-003 „GPU process isn't usable”), PNG renderuje przewodniczący po merge; rev. 3.10: `.mcp.json` z gołym `${CLAUDE_PLUGIN_ROOT}` (rc6→rc7, §21), rc10: `gates.redact` usuwa katalog domowy z zapisywanego wyjścia bramek (trafia do `.council/reports/` i do publicznego repo), doctor pokazuje błąd sondy zamiast `ready` (rc8), `init` wybiera już ściągnięty model Ollama gdy domyślnego brak (`setup.pick_ollama_model`, rc9), grafiki i teksty promo w `docs/social/`; rev. 3.9: Obsidian faza B, `/council:analyze`, polityka delegowania i budżet sesji (§22), fallback do Claude przy braku kwoty (§23); rev. 3.8: instalacja jako plugin (marketplace, SessionStart), `council setup/doctor` z logowaniem, kontrola prywatności, warstwa Obsidian (§20); rev. 3.7: faza 3 — dokumentacja użytkownika EN w `docs/`, `council report`, pierwszy realny epik (sitek-site) przez narzędzia MCP; rev. 3.6: adapter `antigravity` (agy) — następca Gemini CLI dla kont indywidualnych, generuje PNG (19.15); rev. 3.5: faza 2b — trust, defects_after_merge, LESSONS.md, playbooki, compare, why, dissent; rev. 3.4: faza 2 — review/verdict/merge, gates, hook, agenci, rola `assets`, routing wg typu zadania (19.12); rev. 3.3: faza 0.5 i faza 1 wykonane, adapter `copilot`, ustalenia z testu żywego (19.11); rev. 3.2: sekcja 19 — decyzje po przeglądzie Opus/Sonnet, krok 0 i faza 0 wykonane; rev. 3.1: projekt publiczny (open source) + krok 0 w Claude Code; rev. 3.0: przegląd z sześciu perspektyw + 14 ulepszeń (sekcja 16); rev. 2.1: playbooki (15); rev. 2.0: przegląd krytyczny, warstwa spec→kontrakty→DAG, gates, kwoty, baza wiedzy, `/council:analyze`, `council.json` v2) · wszystkie decyzje zamknięte · repo: `super-claude-code`
...
Przy starcie `council-mcp`: `which/where` każdego `cmd`, `<cmd> --version`, `<cmd> --help` → wykrycie flag, `GET {url}/api/tags` dla Ollamy. Wynik w `capabilities.json`; model niedostępny dostaje `enabled:false` i znika z routingu. `council_models` pokazuje to Claude'owi. Od rc25 sonda ma świeżość (`probe_ttl_hours`, §28): przed każdym dispatch `ensure_fresh` porównuje `probed_at` z TTL i przestarzały wynik sonduje ponownie; sonda CLI próbuje też odkryć listę modeli (`models`/`models list`, cicha porażka) i zapisuje wspierane wartości effortu per adapter.
...
| **3** | dokumentacja użytkownika (§12, EN), pozostałe playbooki, `council bench/night/report`, profile, `council_recall`, `/council:spec|architect|docs`, epiki, tag `v1.0` | 3 | **3a wykonana 2026-09-05**: `docs/` (getting-started, reference, adapters, recipes, security), `council report`, epik testowy `sitek-site` (assets→codex, docs→copilot, implement→antigravity; review→merge przez MCP). **Odłożone po v1.0** (świadomie, §14.8/§19.7): `council bench/night`, profile, `council_recall` (pgvector), `/council:spec|architect|docs|analyze`, epiki jako obiekt, playbooki C/F–J, szacunek solo vs council |
| **4** | strojenie routingu na `stats`, CI dla repo docelowych, `council export-audit` | po miesiącu użycia | częściowo 2026-09-05: `/council:analyze` (14.3), Obsidian faza B (§20), polityka delegowania (§22), fallback (§23). Tag `v1.0.0-rc1`. Otwarte: `export-audit`, strojenie routingu na `stats`, `council bench/night`, profile, `council_recall` |
...
### 19.17 GPT-6 Astra jako domyślny model Codexa, rola `3d` (2026-09-06, rc11)

Użytkownik ma w `~/.codex/config.toml` `model = "gpt-6-astra"`, `model_reasoning_effort = "medium"`; plugin przekazuje to jawnie (`-m gpt-6-astra -c model_reasoning_effort="medium" -c model_context_window=256000`), żeby nie zależeć od lokalnej konfiguracji. Nowe pola `ModelConfig.reasoning` (`low|medium|high|xhigh|max`) i `context_window` (tylko codex). Research (`docs/research-*` nie zakładano; źródła: developers.openai.com/api/docs/models/gpt-6-astra, learn.chatgpt.com/docs/config-file/config-advanced, dev.epicgames.com Unreal MCP): id modelu potwierdzone, poziomy wysiłku potwierdzone, **256k to wybór użytkownika** (oficjalnie ~1M), Codex CLI ≥0.153. Oficjalnie brak integracji Blender/Unreal; UE 5.8 ma eksperymentalny serwer MCP z klientem Codex.

Rola `3d`: tekstury (PNG, potęgi dwójki, komplet map), skrypty `bpy` uruchamiane headless, kod Unreal (C++/Blueprint-Python/`.ini`). `setup.detect_tools()` wykrywa Blender (`BLENDER_EXE`, PATH, Program Files) i Unreal (rc12: `UE_ROOT` → lista instalacji Epic Launchera → rejestr → płytki skan **tylko dysków stałych** za `UE_*` do 3 poziomów, np. `D:/GAMES/Unreal/UE_5.8`, najnowsza wersja pierwsza; wynik cache'owany per proces, doctor skanuje od nowa). Lekcja rc12: glob po dysku sieciowym zawiesił pytest na minuty → nigdy nie skanować dysków innych niż DRIVE_FIXED; w testach nie podmieniać `os.name` (na Windows `Path()` wtedy rzuca NotImplementedError i ginie w `except`), wynik idzie do promptu CLI (`prompt_cli.j2`) i do `council_doctor.tools`. Brak narzędzia ≠ `blocked`: wykonawca zostawia skrypt + instrukcję. Routing `3d`: codex → antigravity; `3d` w `ALWAYS_DELEGATE`. Playbook `game-assets` (kontrakt asset-ów u Claude, tekstury i kod jako osobne karty, review sprawdza istnienie/wymiary). Na tej maszynie Blender/UE nie są zainstalowane — ścieżka „bez narzędzi” przetestowana jednostkowo, ścieżka „z narzędziami” tylko przez testy z podstawionymi plikami.
...
- Codex, T-003 (2026-09-05): poprawka CSS grafiki social (kolumna modeli 330→270 px, 42 px odstępu od nagłówka) przeszła, ale headless Edge z workdira zakończył się `GPU process isn't usable` (w T-001 ten sam render działał). Reguła: wykonawca edytuje HTML, a gdy render nie startuje — `blocked`, nie ponawia; PNG renderuje przewodniczący na `main` po merge (`docs/social/posts.md`, sekcja „Re-render”). Profil przeglądarki nigdy w workdirze (190 plików `scope_violation`, usunięte przez Watcher).
...
**Faza A (zrobione):** `obsidian.py` — wykrywanie vaultów z `obsidian.json` (Windows/macOS/Linux), preferencja vaulta zawierającego repo, detekcja Claudiana (`.obsidian/plugins/claudian`), lustro `<vault>/Council/<projekt>/` (MEMORY/HANDOFF/LESSONS/TASKS, `REPORT-*.md`, `reports/`, karty jako notatki z frontmatter + README z Dataview); jeśli repo jest w vaulcie — tylko notatka indeksowa. Wywołanie: po `council_plan`, `council_merge`, `council_handoff`, narzędzie `council_obsidian`, `council obsidian --mirror`. Konfiguracja `obsidian: {vault, folder, mirror, read_context}`; kolejność wyboru vaulta: `council.json` → zmienna `COUNCIL_OBSIDIAN_VAULT` (jeden vault dla wszystkich projektów użytkownika, zalecane) → vault zawierający repo → otwarty vault. U pierwszego użytkownika: dedykowany vault `WORKSPACE/obsidian-council` z `Dashboard.md` (Dataview: projekty, blocked, review, merged). `doctor` i hook SessionStart mówią, czy Obsidian/Claudian są, i sugerują instalację.

**Faza B (wykonana 2026-09-05, poza pkt 5):** `vault.py` — `council_context` (read_context, Plan.md, DECISIONS.md, Questions.md, Decisions/*.md, notatki `#council/spec`; limit 60k znaków), `sync_decisions` przy `council_plan` (event `memory_from_vault`), inbox: `blocked` → `inbox/T-xxx.md` z polem `answer:`; `council_status` stosuje odpowiedzi (`answer_from_vault`) i przenosi notatkę do `inbox/answered/`; `council init --obsidian` pisze zestaw komend Claudiana i blok w `CLAUDE.md` vaulta. Dedykowany vault przez `COUNCIL_OBSIDIAN_VAULT`.

**Faza B (oryginalna propozycja):** (1) `read_context` — notatki vaulta (spec, ADR) czytane przez plannera przed kartami, z cytowaniem źródła; notatki `#council/spec` automatycznie; (2) sekcja `## Decisions` w notatce projektu scalana do `MEMORY.md` przy planowaniu (vault wygrywa, event `memory_from_vault`); (3) `blocked` → notatka `inbox/T-xxx.md` z polem `answer:`; wypełnienie w Obsidianie → hook SessionStart → `council_answer` (to jest `/council:inbox` z 16.5 w vaulcie); (4) zestaw komend Claudiana wołający ten sam serwer MCP; (5) linki kart do dotkniętych plików i lekcji — graf pokazuje, który model dotykał czego. **Nie planujemy:** zapisu do vaulta przez wykonawców, zależności od Obsidian Sync.

## 21. Instalacja i higiena publikacji (2026-09-05)

- Instalacja przetestowana 2026-09-05 na drugiej maszynie-katalogu (`nuco-energy-flow`): klon przez SSH padał (brak klucza), więc źródło = URL HTTPS; `commands/agents/hooks` ładują się automatycznie i nie mogą być w manifeście; poprawki manifestu wymagają nowej wersji (cache). Wersja `1.0.0-rc2` ładuje się poprawnie (`Status: enabled`).
- Błędy narzędzi (2026-09-05): SDK mcp 2.x ukrywa wyjątki inne niż `ToolError` za „Error executing tool X” — użytkownik widział to przy `council_doctor` w innym projekcie, bez przyczyny. Od rc5 każde narzędzie jest opakowane (`_tool`): wyjątek → `ToolError` z typem i treścią; `council_ping` (bez konfiguracji) pokazuje root, env i `uv`; root repo: `COUNCIL_REPO_ROOT` tylko gdy to istniejący katalog (host może przekazać `${VAR}` bez rozwinięcia) → `CLAUDE_PROJECT_DIR` → cwd; brak `council.json` = czytelny komunikat z ścieżkami.
- Dokumentacja poprawiona (2026-09-05): instalacja pluginu **nie daje** polecenia `council` w PATH (CLI żyje w venv cache pluginu) — README/getting-started/reference prowadzą przez `/council:setup`, `/council:doctor` i narzędzia MCP (`council_setup`, `council_doctor`, `council_obsidian(kit=true)`); forma terminalowa tylko jako `uv run --directory <cache> council …` lub `uv run council …` w klonie.
- `council init` uruchomiony z cache pluginu pisał `.mcp.json` projektu ze **stałą ścieżką do jednej wersji cache** (rc2) — po aktualizacji pluginu Claude łączył się ze starym serwerem bez nowych narzędzi (`council_doctor` „nie istnieje”). Od rc4 `init` pomija `.mcp.json`, gdy `plugin_dir` leży w `.claude/plugins/cache` (serwer daje plugin); wpis w `nuco-energy-flow` usunięty ręcznie. Wersja `1.0.0-rc4`.
- Pierwsza sesja użytkownika z pluginem (2026-09-05): komendy pokazywały się jako `/council:council:status` — podkatalog `commands/council/` staje się drugą przestrzenią nazw; pliki przeniesione do `commands/*.md`. Serwer MCP nie połączył się (`CONNECTION_CLOSED`): `uv` był w PATH rejestru, ale nie w środowisku uruchomionego procesu Claude Code (PATH zmieniony po starcie) — wymagany restart; `.mcp.json` i hooki używają `${COUNCIL_UV:-uv}`, żeby dało się wskazać uv bez PATH. Dodano `/council:doctor` + narzędzie `council_doctor`. Wersja `1.0.0-rc3`.
- Plugin: `.claude-plugin/marketplace.json` → `/plugin marketplace add sitkowsp/super-claude-code` + `/plugin install council@super-claude-code`. `.mcp.json` używa **gołego** `${CLAUDE_PLUGIN_ROOT}` (rc7). Lekcja rc6/rc7: Claude Code podstawia `${CLAUDE_PLUGIN_ROOT}` dla serwerów pluginu literalnie; formy z domyślną wartością (`${A:-x}`, tym bardziej zagnieżdżone `${A:-${B}}`) działały w desktopie tylko dlatego, że zmienna była też w env procesu, a w `claude -p` dawały `--directory .` → `CONNECTION_CLOSED`. Plik repo jest manifestem pluginu; w klonie serwer dla projektu zapisuje `council init` (ścieżka bezwzględna), a otwarty jako projekt dev checkout pokazuje ten wpis jako pominięty (brak zmiennej) — to zamierzone. Hook **SessionStart** uruchamia `council session-start`: `init` gdy brak `.council/`, jedna linia o gotowych wykonawcach, wzmianka o `HANDOFF.md` i Obsidianie.
...
- Grok: **zweryfikowany 2026-09-05** — oficjalne xAI Grok Build CLI (`npm i -g @xai-official/grok`, binarka `grok` 1.0.13, logowanie kontem grok.com/X przez `grok login`). Headless: `-p/--single`, `--cwd`, `--always-approve`, `--output-format plain`; czyta `AGENTS.md`. Test żywy: odpowiedź w 6 s, zadanie z zapisem plików i poprawnym REPORT.md w 34 s. Generowanie obrazów: has image_gen tool but it returned 403 on this plan (credits/subscription); falls back to drawing with Pillow. Włączony w routingu (implement/review/assets, na końcu listy do czasu zebrania `stats`). Dokument: `docs/research-grok-cli-2026-09.md`.
...
## 22. Polityka delegowania i budżet sesji (2026-09-05)

Cel nadrzędny użytkownika: **oszczędzać tokeny Claude** — okno użycia (5 h) jest zasobem deficytowym, subskrypcje wykonawców są opłacone niezależnie.

- `delegation` w `council.json`: `mode` (`auto` — deleguj bez pytania gdy reguły tak mówią; `ask` — zawsze potwierdź; `off`), progi `min_lines`=40 / `min_files`=2, `always_delegate_roles` (docs, assets, chores, review, data), `keep_for_claude` (kontrakty, integracja, merge, bezpieczeństwo, mały hotfix), `ask_when_unsure`.
- `policy.decide()` — funkcja czysta: szwy → Claude; rola z listy → deleguj; rozmiar ≥ próg → deleguj; mała zmiana w jednym pliku → Claude; reszta → **pytanie do użytkownika** (zredukowane do przypadków granicznych). Narzędzie `council_should_delegate`.
- Przypomnienie polityki w hooku SessionStart (jedna linia w kontekście), planner ma zakaz planowania kart dla Claude poza kontraktami/integracją/merge.
- Budżet: zegar sesji (`.council/.session_started`, reset przy `source=startup`), ostrzeżenie po `warn_after_minutes`=210 z 300 w hooku SessionStart i UserPromptSubmit (co 30 min), narzędzie `council_budget`, komenda `/council:offload` — reszta pracy sesji idzie do wykonawców + `HANDOFF.md`, Claude zostaje z review/merge. Claude Code nie udostępnia API limitu, więc heurystyka czasowa jest świadomym przybliżeniem.

## 23. Fallback do Claude (2026-09-05)

Gdy wykonawca nie odpowiada, wyczerpał kwotę lub jest niedostępny: `scheduler.classify_failure()` (429/quota/rate limit/credits → `quota`; ENOTFOUND/503/not logged in → `unavailable`; kod ≠ 0 bez logu lub timeout → `no_response`; prawdziwy błąd zadania → brak fallbacku). Model dostaje `cooldown_until` w `stats.json` (routing go pomija, `fallback` w konfiguracji: `cooldown_minutes`=60), zadanie wraca do `queued` z `assigned_to` = `fallback.model` (domyślnie `cheap` = `claude -p`), `fallbacks`+1 (limit `max_fallbacks`=1), eventy `cooldown` i `fallback`. Bez możliwego fallbacku — event mówi, że orkiestrator (Claude) ma przejąć zadanie. `council_ask` ma ten sam fallback (`fallback_from` w odpowiedzi). Test: `tests/test_fallback.py`.

## 24. Opcje nadzorcy: asystent planowania/recenzji i Claude jako koder (2026-09-06, rc13)

Analiza (sesja 2026-09-06): nadzorca to dowolny klient MCP, więc Codex/Antigravity *mogłyby* nim być, ale wymagałoby to drugiej warstwy komend/hooków i odwracało sens polityki tokenów. Decyzja użytkownika: nadzorcą zostaje Claude Code, dodajemy opcje (domyślnie wyłączone).

- `chair.plan_assist` (model|null): `council_plan(goal, draft=true)` → `render.plan_draft` (cel, `analyze.render`, playbook, MEMORY, notatki vault, istniejące zadania, never_share) → `make(model).ask` → `_extract_json_array` (pierwsza tablica JSON, reszta ignorowana; wyjście = dane) → wspólna walidacja `_validate_cards` (ta sama co zapis) → zwrot `draft` + `errors`, **bez zapisu**; id i `assigned_to` nadaje serwer. Claude i użytkownik zatwierdzają, potem zwykłe `council_plan(tasks)`.
- `chair.review_assist` (model|null): `council_review` dokleja `assistant_review` (`render.review_assist`, ≤12 linii, diff jako dane); błąd asystenta nie blokuje recenzji; asystent ≠ autor zmiany.
- `chair.coder` (`executors`|model): `CouncilConfig.candidates()` stawia model na pierwszym miejscu tylko dla `implement`/`refactor`. Szablon ma `fable` (`claude-sub`, `claude-fable-5-1`, `effort: medium`, `enabled: false`); `setup.set_chair(coder="fable")` włącza model. `ModelConfig.effort` → `claude -p --effort`.
- `COUNCIL_EXECUTOR=1` w env każdego procesu wykonawcy (`adapters.cli.executor_env`); hooki `events`/`session-start` wychodzą od razu, gdy ją widzą — inaczej `claude -p` jako wykonawca odpalałby hook SessionStart pluginu i robił `init` w katalogu roboczym. `--bare` odpada: pomija logowanie OAuth ("Not logged in").
- Przełączanie: `/council:chair`, `council_setup(coder=…, plan_assist=…, review_assist=…)`, `council setup --coder/--plan-assist/--review-assist`; `set_chair` waliduje cały config przed zapisem; `doctor`/`setup` drukują linię `chair:`.
- Uczciwość: `coder: fable` zużywa okno Claude (ten sam limit 5 h), oszczędza kontekst sesji i daje równoległość; realną oszczędność dają asystenci. Testy: `tests/test_chair.py`; akceptacja żywa (repo tymczasowe, driver MCP stdio): szkic planu przez Astrę (2 karty, 30 s), zadanie `implement` wykonane przez Fable, `assistant_review` od Codexa, verdict + merge, powrót do domyślnych. Obserwacja: w repo testowym `scope_violation: .gitattributes, .gitignore` — pliki zapisane przez `council init` po pierwszym commicie były nieśledzone; wykonawca ich nie tworzył (log), diff dotknął tylko `calc.py`. W realnym projekcie pliki `init` są scommitowane; flaga jest wtedy nieobecna (sitek-site). Lekcja rc13 (testy): test, który kończy się z zadaniem schedulera jeszcze w toku (drugi job po fallbacku), zostawia subprocess git i task do posprzątania pytest-asyncio — na Windows (Proactor) sporadycznie wieszało to całą suitę (3 razy w tej sesji, zawsze po 31 testach w `-q`). Reguła: test schedulera zawsze czeka na `s.jobs[...]` przed powrotem.

## 25. Szacunek zaoszczędzonych tokenów Claude (2026-09-07, rc15)

`stats.py`: `SAVINGS[role] = (baza, na_linię)` — implement/refactor 1500+60/linię, docs 800+40, assets/3d 2500+20 (Claude nie wygeneruje obrazu, stała za rundy, które by kosztował), review/chores/adversary 600+20, data 800+30, contract 0; `ASSIST_SAVINGS`: szkic planu 3000, streszczenie recenzji 800. `on_merge` liczy zadanie raz (`Stats.counted_tasks`) na podstawie `git diff --stat base...branch` zebranego w `council_merge` **przed** merge; `on_assist` zlicza użycia asystentów. `savings_summary`/`savings_md` → `council_stats.savings`, nowe `council_savings(backfill)` (backfill: eventy `merged` z `commit` → `git diff --stat commit^1 commit`), sekcja w `council report`, notatka `Council/<project>/Savings.md` z frontmatter (`council_savings`, `tokens_saved`, `tasks`, `lines`…) i tabela sumująca projekty na Dashboardzie, jedna linia w hooku SessionStart. Uczciwość: to heurystyka, nie pomiar — CLI nie raportują tokenów, tokeny Claude na review/merge nie są odejmowane; stałe są celowo ostrożne i opisane w `method`. `_frontmatter` renderuje bool jako `true/false` (Dataview).

## 26. Dwa konta Claude: profile dla wykonawców `claude -p` (2026-09-07, rc16)

Fakty: Claude Code trzyma login per katalog konfiguracji (`CLAUDE_CONFIG_DIR`); `claude auth status` zwraca JSON z `loggedIn/email/orgName/subscriptionType`; `claude auth login` jest tylko interaktywne (przeglądarka, wybór organizacji); brak API limitu — wyczerpanie widać jako błąd (`QUOTA_RE`). Aplikacja desktop używa katalogu domyślnego i nie przełączy konta w trakcie sesji.

Decyzja: automatyka **tylko dla wykonawców**. `claude_profiles: {name: dir|null}`, `ModelConfig.profile` → `config_dir` (walidator), adapter ustawia `CLAUDE_CONFIG_DIR` (`executor_env(config_dir)`). `Fallback.by_model` = łańcuch per model, `Fallback.targets()`; `Scheduler.pick_fallback` (enabled, privacy, nie w cooldownie) używany i dla zadań, i dla `council_ask`. `parse_reset` czyta z komunikatu limitu czas resetu („resets 3pm", „resets at 14:30", „resets in 45 min") → `cooldown_until` na ten moment, inaczej `cooldown_minutes`. `setup.add_profile(name)` tworzy `~/.claude-profiles/<name>`, klony `<model>-<name>` (disabled, `profile`), wstawia je na początek łańcucha i do routingu; `profiles_status` (plik `.credentials.json` + `claude auth status` per profil), `enable_logged_in_profiles`, `remove_profile`, `switch_hint`. Narzędzie `council_accounts(add, verify, remove)`, komenda `/council:accounts`, wiersze w `doctor`, `switch_hint` w `council_budget` i `/council:offload`, dopisek w hooku budżetu. Nadzorca: tylko przepis ręczny (handoff → zamknij → `claude auth login`/`CLAUDE_CONFIG_DIR` → otwórz). Zastrzeżenie o zasadach użytkowania wypisywane przez narzędzie i w README. Test symulacyjny: `fable` z komunikatem limitu „resets 11pm" → zadanie kończy `fable-alt`, cooldown `fable` do 23:00 lokalnie; test żywy z dwoma prawdziwymi kontami niewykonany (jedno konto na tej maszynie) — wykonano ścieżkę „profil niezalogowany" (`Not logged in` → `unavailable` → fallback na profil domyślny).

## 27. Automat oceny złożoności: tier modelu + effort (2026-09-07, rc24)

`complexity.assess(task)` — czysto deterministyczny (bez wywołań modeli): waga roli (refactor 4, implement 3, 3d 2, assets/data 1), rozpiętość scope (liczba globów, `**` +3, katalog +2), długość celu, liczba kryteriów akceptacji, `depends_on`, słowa kluczowe złożoności (refactor/migrat/concurren/parser/security/solver…, PL+EN) i prostoty (typo/rename/readme/bump/format…), oraz `attempt` (odrzucenie podnosi poziom przy ponowieniu). Progi: ≤2 simple, ≤7 standard, dalej complex. Skutki przy dispatch (`delegation.auto_effort`, domyślnie on): wybór pierwszego kandydata o `tier` low (simple) / high (complex) w ramach istniejącego routingu (remisy wg kolejności; `assigned_to` nadal nadrzędne), oraz effort low/medium/high przekazywany per zadanie — codex `-c model_reasoning_effort`, claude-sub `--effort` (nadpisuje statyczne cfg tylko na czas biegu; `ask`/probe używają cfg). Zapis: `Task.complexity/effort`, powody w evencie `dispatched` (→ `council_why`), podgląd per karta w wyniku `council_plan`. Tiery w szablonie: local/cheap=low, codex/fable=high, reszta standard.

## 28. Dzienna sonda dostępności modeli (2026-09-08, rc25)

Problem: sonda odpalała się raz na proces serwera MCP — w długo żyjącej sesji desktopowej `capabilities.json` miewał wiele dni, a wybór modelu/effortu (§27) pracował na nieaktualnej wiedzy o dostępności.

- `probe_ttl_hours` (top-level w `council.json`, domyślnie 24; 0 = zachowanie sprzed rc25). `probe.load/age_hours/is_fresh/ensure_fresh`; `_Runtime.caps()` używa `ensure_fresh`, a `council_dispatch` woła `caps()` przed każdą delegacją — koszt zerowy przy świeżym wyniku, raz na dobę pełna sonda ("pierwsze użycie dnia").
- Kolejność w `ensure_fresh`: świeża kopia w pamięci → świeży plik z dysku (werdykty `enabled` z pliku aplikowane do cfg; inny proces mógł już sondować) → pełna sonda. Przed ponowną sondą `enabled` każdego modelu wraca do wartości z council.json — dostawca, który wyzdrowiał od wczoraj, może się ponownie włączyć (wcześniej `enabled and res.enabled` trzymało go martwym do restartu).
- Odkrywanie modeli per CLI: kandydackie podkomendy (`models`, `models list`) z timeoutem 15 s i cichą porażką; parser `parse_model_list` bierze pierwszy token linii wyglądający jak id (musi mieć cyfrę/separator — proza i nagłówki odpadają), limit 50. Stan faktyczny 2026-09-08: `agy models` zwraca pełną listę (14 modeli Gemini), codex/copilot/grok/gemini/claude nie mają listowania → pusta lista = config autorytatywny. Ollama jak dotąd przez `/api/tags`.
- Rejestr effortów per adapter w `Capabilities.efforts`: codex i claude-sub `low…max` (claude-sub bramkowany wykrytą flagą `--effort`); pozostałe puste. `complexity.assess` wybiera tylko low/medium/high, więc oba zbiory je pokrywają.
- `council_models` pokazuje `probe_age_hours`, `probe_ttl_hours`, `tier`, `available_models`, `efforts` i `warning`, gdy skonfigurowany model nie występuje na niepustej odkrytej liście; `council_probe` = wymuszona sonda natychmiast. Hook SessionStart drukuje wiek sondy, gdy przekracza TTL (sam nie sonduje — pozostaje tani).
- Testy: `tests/test_probe_ttl.py` (świeżość/TTL 0/zepsuty plik; ensure_fresh nie sonduje przy świeżej pamięci/pliku, przy przestarzałym sonduje z przywróconym `enabled`; parser listy; efforty codexa na atrapie CLI). Smoke żywy: pełna sonda 7 modeli ~19 s.

## 29. Adapter `cursor` — Cursor CLI jako opcjonalny wykonawca (2026-09-08, rc26)

Decyzja użytkownika: integracja jako opcja (nie ma konta Cursor, ale binarka ma być wykrywalna od instalacji). Research 2026-09-08: binarka `agent` (alias `cursor-agent` — używamy aliasu, bo `agent` jest zbyt generyczne na PATH), instalacja skryptem `curl https://cursor.com/install -fsS | bash` / wariant PowerShell (nie npm — brak problemu shimów `.cmd`), headless `-p/--print` + `--output-format text|json` + `--force|--yolo` (zapis plików), `--model <id>`, czyta natywnie `AGENTS.md`, `CLAUDE.md` i `.cursor/rules`; `agent models` listuje modele konta (Composer 2.5, GPT‑5.6, Opus 5, Gemini 3.1 Pro, Grok 4.6…). Logowanie: konto Cursor lub `CURSOR_API_KEY`. Zgłaszane zawieszki `-p` na forum → nasz twardy budżet (kill po `hard_minutes`, `failed: no_final_report`) już to pokrywa.

Realizacja: `AdapterName` + `cursor`; wpisy w `_ASK_ARGV`/`_RUN_ARGV` (`-p {prompt} --output-format text`, cwd=workdir bez flagi), `_APPROVAL` (`--force`, potem `--yolo`), `_READS_CHARTER_FILE` (bez wklejania Charteru), `_LIST_MODELS` (`models` → dzienna sonda §28), `--model` przekazywany jawnie jak dla antigravity (niezależnie od wykrytych flag); brak pokrętła effortu — automat złożoności (§27) wpływa tylko przez tier. Szablon: model `cursor` (`cmd: cursor-agent`, `enabled: false`, `model: composer-2.5`, role implement/refactor/docs/review, public) + pozycje w routingu za copilotem. `setup.CATALOG` + `login_state` (`CURSOR_API_KEY` = pewne; login subskrypcyjny bez taniego sprawdzenia → None). Testy: `tests/test_cursor.py` (argv/approval/--model, cicha porażka odkrywania na atrapie, parser `agent models`, szablon disabled + kandydaci po włączeniu, doctor: brak binarki → dokładna komenda instalacji, API key → logged_in). Weryfikacja żywa: **tylko wykrywanie** (throwaway repo: sonda OFF "cursor-agent not on PATH", doctor z komendą instalacji); pełny bieg wykonawcy do zweryfikowania przez posiadacza konta Cursor.
