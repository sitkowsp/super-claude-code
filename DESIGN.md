...
- Plugin: `.claude-plugin/marketplace.json` → `/plugin marketplace add sitkowsp/super-claude-code` + `/plugin install council@super-claude-code`. `.mcp.json` używa `${CLAUDE_PLUGIN_ROOT:-.}` (rc6: Claude Code **nie rozwija zagnieżdżonych** domyślnych `${A:-${B}}` w `.mcp.json` — literal trafiał do `uv --directory` i serwer padał z `CONNECTION_CLOSED`; hooki idą przez shell, więc tam zagnieżdżenie działa). Ten sam plik działa w instalacji pluginu (`CLAUDE_PLUGIN_ROOT`) i w klonie (cwd = repo). Hook **SessionStart** uruchamia `council session-start`: `init` gdy brak `.council/`, jedna linia o gotowych wykonawcach, wzmianka o `HANDOFF.md` i Obsidianie.
