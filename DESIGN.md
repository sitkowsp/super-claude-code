...
Rola `3d`: tekstury (PNG, potęgi dwójki, komplet map), skrypty `bpy` uruchamiane headless, kod Unreal (C++/Blueprint-Python/`.ini`). `setup.detect_tools()` wykrywa Blender (`BLENDER_EXE`, PATH, Program Files) i Unreal (`UE_ROOT`, `Epic Games/UE_*` → `UnrealEditor-Cmd`), wynik idzie do promptu CLI (`prompt_cli.j2`) i do `council_doctor.tools`. Brak narzędzia ≠ `blocked`: wykonawca zostawia skrypt + instrukcję. Routing `3d`: codex → antigravity; `3d` w `ALWAYS_DELEGATE`. Playbook `game-assets` (kontrakt asset-ów u Claude, tekstury i kod jako osobne karty, review sprawdza istnienie/wymiary). Na tej maszynie Blender/UE nie są zainstalowane — ścieżka „bez narzędzi” przetestowana jednostkowo, ścieżka „z narzędziami” tylko przez testy z podstawionymi plikami.
