...
(dry run). Chair options in $ARGUMENTS map to the same tool: `--coder <name>`, `--plan-assist <model|off>`,
`--review-assist <model|off>` (see `/council:chair`). Print the `chair` line first. Show the executor table, then the install commands (run for the user when installing,
